...
class RRT():
  ...
  def traj(self,frame):
     ...
     ar=Node(random.randrange(0,1600), random.randrange(0,1200))
     print ar.x,ar.y
     nin1=self.indexofNearest(self.nlist,ar)
     theta1=math.atan((self.nlist[nin1].y-ar.y/self.nlist[nin1].x-ar.x))
     ar12=Node(self.nlist[nin1].x+self.r*math.cos(math.pi+theta1),self.nlist[nin1].y+self.r*math.sin(theta1))
     ar12.parent=nin1
     #print ar12.x,ar12.y
     reso=0.1
     ...
     while k<=self.r:
       print k
       p=self.nlist[nin1].x+k*(0.1*math.cos(math.pi+theta1))
  ...
  def goal_reach(self,x,y):
   if x<1080and y<1027: 
    if self.frame[int(x),int(y)][0]<=30 and self.frame[int(x),int(y)][1]>250 and self.frame[int(x),int(y)][2]<=30:
      self.goal=[x,y]
      return 1
    else:
      return 0
  ...
  def obstaclecheck(self,theta,x,y):
   ...
   for v in np.arange(0, self.r+1,reso):
     ...
     if self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][0]<=30 and self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][1]<=30 and self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][2]<=30:
        print x, y,1
        return 0
     if self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][0]>=225 and self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][1]<=30 and self.frame[int(x+v*math.cos(theta)),int(y+v*math.sin(theta))][2]<=30:
        print x,y,2
        return 0
